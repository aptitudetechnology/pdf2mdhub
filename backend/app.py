...
def upload_document():
    ...
    try:
        ...
        if 'pdf_file' not in request.files:
            logger.error("Error: No 'pdf_file' part in request.files.")
            logger.info("--- END: POST /api/documents Request (400 - No file part) ---")
            return jsonify({'error': 'No file provided'}), 400

        # CHANGE THIS LINE from 'file' to 'pdf_file'
        ...
        if not allowed_file(file.filename):
            ...
            return jsonify({'error': 'File type not allowed. Only PDFs are accepted.'}), 400
        ...
        # Save file temporarily (before committing DB entry)
        file.save(file_path)
        ...
        db.session.flush()  # Get the ID so we can associate tags/metadata, and reference for logging
        logger.info(f"Document record initialized in DB (not yet committed): ID={document.id}, Original Name={document.original_name}")

        # --- ADDED: PDF to Markdown Conversion ---
        markdown_content = convert_pdf_to_markdown(file_path)
        if markdown_content is not None:
            document.markdown_content = markdown_content
            document.processed_date = datetime.utcnow()
            document.status = 'processed' # Update status to processed
            logger.info(f"PDF converted to Markdown successfully for document ID: {document.id}. Status set to 'processed'.")
        else:
            document.markdown_content = "Failed to convert PDF to Markdown."
            document.status = 'failed' # Set status to failed
            logger.warning(f"Failed to convert PDF to Markdown for document ID: {document.id}.")
        ...
        if 'file_path' in locals() and os.path.exists(file_path): # file_path might not be defined if error occurs very early
            os.remove(file_path)
...
def list_documents():
    ...
    try:
        ...
        if tag:
            # THIS WAS THE INDENTATION ERROR: The next line needs to be indented
...
def update_document(document_id):
    ...
    try:
        ...
        if 'markdown_content' in data: # Allows manual update of markdown, or re-processing via API
            document.markdown_content = data['markdown_content']
            document.processed_date = datetime.utcnow() # Update processed date on markdown content change
            # If status was 'uploaded' and markdown content is now provided, set to processed
