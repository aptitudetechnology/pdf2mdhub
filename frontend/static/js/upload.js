...
def upload_document():
    # 1. Handle file upload - accept 'file' or fallback to 'pdf_file'
    ...
    try:
        ...
        # Accept tags as JSON string or as multiple 'tags' fields
        raw_tags = request.form.get('tags')
        if raw_tags:
            try:
                tag_names = json.loads(raw_tags) if isinstance(raw_tags, str) else []
            except json.JSONDecodeError:
                tag_names = []
                current_app.logger.warning("Invalid tag format received.")
        else:
