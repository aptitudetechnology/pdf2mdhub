...
UPLOAD_URL = "http://127.0.0.1:5050/api/upload"  # Replace with your actual URL.
...
def run_upload_test(
    ...
    try:
        ...
        try:
            ...
            print(
                f"Test '{test_name}' FAILED: Could not decode JSON from response. Response content: {response.text}"
            )
        ...
        if expected_message_substring:
            ...
            assert (
                expected_message_substring in actual_message
        ...
        if expected_status >= 200 and expected_status < 300 and check_document_data:
            ...
            assert (
                returned_document
            ), f"Test '{test_name}' FAILED: No 'document' object in successful response."
            for key, expected_value in check_document_data.items():
                ...
                if key == "tags":  # Special handling for tags, order might differ
                    ...
                    expected_value_set = (
                        set(expected_value) if expected_value else set()
                    )
                    assert (
                        actual_value_set == expected_value_set
                    ...
                    for meta_key, meta_val in expected_value.items():
                        assert (
                            returned_document["document_metadata"].get(meta_key)
                            == meta_val
